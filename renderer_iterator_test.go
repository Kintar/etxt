...
func TestStrIterator(t *testing.T) {
	...
	gotRune = iter.Next()
	if gotRune != 'A' {
		testFailRunes(t, 'A', gotRune)
	}
	gotRune = iter.Next()
	if gotRune != -1 {
		testFailRunes(t, -1, gotRune)
	}
	expected, got = "", iter.UntilNextLineBreak()
	if got != expected {
		testFailStr(t, expected, got)
...
func TestGlyphsIterator(t *testing.T) {
	iter := newGlyphsIterator([]GlyphIndex{1, 2, 3, 4}, false)
	for n, expected := range []GlyphIndex{1, 2, 3, 4} {
		got, done := iter.Next()
		if done {
			t.Fatalf("test#%d unexpectedly done", n)
		}
		if got != expected {
...
func TestGlyphsIteratorReverse(t *testing.T) {
	iter := newGlyphsIterator([]GlyphIndex{1, 2, 3, 4}, true)
	for n, expected := range []GlyphIndex{4, 3, 2, 1} {
		got, done := iter.Next()
		if done {
			t.Fatalf("test#%d unexpectedly done", n)
		}
